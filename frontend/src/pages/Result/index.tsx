...
const 结束状态 = ['success', 'failed', 'stopped', 'partial_success']
...
export function TaskRunPage() {
  ...
  const 已结束 = useMemo(() => {
    if (!任务) return false
    return 结束状态.includes(任务.status)
  }, [任务])
  ...
  const 刷新任务 = async (静默 = false) => {
    ...
    try {
      ...
      set任务(data)
      已结束引用.current = 结束状态.includes(data.status)
      set错误('')
    } catch (error) {
        ...
        <>
          ...
          <div className="card log-card">
            ...
            <div className="log-stream" ref={日志容器引用}>
              {日志列表.length ? (
                日志列表.map((line, index) => (
                  <div className="log-row" key={`${index}-${line.slice(0, 32)}`}>
                ...
                <div className="log-empty">
                  当前尚未收到日志输出。若任务已经运行过一段时间，当前页面只能接收打开页面之后的新日志。
                </div>
              )}
            </div>
          </div>
